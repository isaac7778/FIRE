...
from scale_rl.agents.simba.projection import orthogonal_project_network
...
class SimbaConfig:
    ...
    ns_steps: int
    reinit_on_load: bool
    reinit_update_steps: List[int] = field(
...
class SimbaAgent(BaseAgent):
    ...
    def _orthogonally_project_network(self):
        # NOTE: Most operations here are done in-place, so original parameters will be lost

        # 1. Project the networks
        projected_actor = orthogonal_project_network(self._actor, self._cfg.ns_steps, self._cfg.scaler_type)
        projected_critic = orthogonal_project_network(self._critic, self._cfg.ns_steps, self._cfg.scaler_type)
        
        # 2. Reinit networks
        self._init_network()

        # 3. Copy projected networks
        if self._cfg.reinit_type == 'fire':
            self._actor = self._actor.replace(params=projected_actor.params)
            self._critic = self._critic.replace(params=projected_critic.params)
            self._target_critic = self._target_critic.replace(params=projected_critic.params)
        else:
            raise ValueError

    def _snp_network(self, shrink_strength=0.8):
        (
            _,
            new_actor,
            new_critic,
        ...
        def snp_fn(original, new):
            return original*(1.0-shrink_strength) + new*shrink_strength
        ...
        self._actor = new_actor.replace(params=new_actor_params)
        self._critic = new_critic.replace(params=new_critic_params)
        self._target_critic = new_target_critic.replace(params=new_critic_params)
    ...
    def update(self, update_step: int, batch: Dict[str, np.ndarray]) -> Dict:
        ...
        if update_step in self._cfg.reinit_update_steps:
            if self._cfg.reinit_type == 'full_reset':
                self._init_network()
            elif self._cfg.reinit_type == 'fire':
                self._orthogonally_project_network()
            elif self._cfg.reinit_type == 'snp':
                self._snp_network()
            else:
                raise ValueError
    ...
    def load(self, path: str) -> None:
        ...
        if self._cfg.reinit_on_load:
            print(f"Reinitializing with {self._cfg.reinit_type}...")
            if self._cfg.reinit_type == 'full_reset':
                self._init_network()
            elif self._cfg.reinit_type == 'fire':
                self._orthogonally_project_network()
